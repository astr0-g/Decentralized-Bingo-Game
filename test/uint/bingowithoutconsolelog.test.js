    ...
    : describe("Bingo test", function () {
          ...
          describe("Construtor", () => {
              ...
              it("Test setConfig", async () => {
                  ...
                  await Bingo.setConfig(joinDuration, turnDuration, betAmountForBINGO, false, 10)
                  ...
                  const returnBet = await Bingo.returnBet()
                  const maxPlayerNum = await Bingo.maxPlayerNum()
                  ...
                  assert.equal(returnBet.toString(), "false")
                  assert.equal(maxPlayerNum.toString(), "10")
                  await expect(
              ...
              it("Test start new game with transfer BingoToken and player cannot join the same game.", async () => {
                  ...
                  await BingoToken.mint()
                  await BingoToken.approve(Bingo.address, fee)
                  await expect(Bingo.joinCurrentGameWithBet(gameRoundNow)).to.be.revertedWith(
                      "error__inGameAlready"
                  )
              })
              it("Test start new game and check player game board", async () => {
                  ...
                  assert.equal(result.toString(), 1 * 10 ** 18)
                  const gameRoundNow = await Bingo.gameRoundNow()
                  const playercards = await Bingo.getPlayerGameBoard(deployer, gameRoundNow)
              ...
              it("only one player join a game and get their bet token back", async () => {
                  ...
                  // console.log("player wait for the result")
                  await Bingo.drawWinner(gameRoundNow)
                  await Bingo.connect(player1).claimPrize(gameRoundNow)
              ...
              it("two players join same game and check their awards", async () => {
                  ...
                  // console.log(`host call drawWinner function`)
                  // console.log("player wait for the result")
                  await Bingo.drawWinner(gameRoundNow)
                  // await Bingo.connect(player1).claimPrize(gameRoundNow)
                  // await Bingo.connect(player2).claimPrize(gameRoundNow)
                  const player1balance = await BingoToken.balanceOf(player1.address)
                  ...
                  const player1win = await Bingo.checkWinner(gameRoundNow, player1.address)
                  const player2win = await Bingo.checkWinner(gameRoundNow, player2.address)
              ...
              it("three players join same game and check their awards for ten times to check accurate gas cost", async () => {
                  for (i = 0; i < 10; i++) {
                      ...
                      // await Bingo.connect(player1).claimPrize(gameRoundNow)
                      // await Bingo.connect(player2).claimPrize(gameRoundNow)
                      // await Bingo.connect(player3).claimPrize(gameRoundNow)
                      const player1balance = await BingoToken.balanceOf(player1.address)
                      const player2balance = await BingoToken.balanceOf(player2.address)
                      ...
                      const player1win = await Bingo.checkWinner(gameRoundNow, player1.address)
                      const player2win = await Bingo.checkWinner(gameRoundNow, player2.address)
                      const player3win = await Bingo.checkWinner(gameRoundNow, player3.address)
                      // console.log(`player 1 win, Prize:${player1win.toString()}`)
                      // console.log(`player 2 win, Prize:${player2win.toString()}`)
              ...
              it("four players join same game and check their awards for ten times to check accurate gas cost", async () => {
                  for (i = 0; i < 10; i++) {
                      ...
                      // await Bingo.connect(player2).claimPrize(gameRoundNow)
                      // await Bingo.connect(player3).claimPrize(gameRoundNow)
                      // await Bingo.connect(player4).claimPrize(gameRoundNow)
                      // await Bingo.connect(player5).claimPrize(gameRoundNow);
                      // await Bingo.connect(player6).claimPrize(gameRoundNow);
                      const player1balance = await BingoToken.balanceOf(player1.address)
                      const player2balance = await BingoToken.balanceOf(player2.address)
                      const player3balance = await BingoToken.balanceOf(player3.address)
                      ...
                      const player2win = await Bingo.checkWinner(gameRoundNow, player2.address)
                      const player3win = await Bingo.checkWinner(gameRoundNow, player3.address)
                      const player4win = await Bingo.checkWinner(gameRoundNow, player4.address)
